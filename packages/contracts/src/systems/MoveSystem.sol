...
// components: ["EncounterComponent", "MonsterTypeComponent", "PositionComponent"]
...
contract MoveSystem is System {
    ...
    function executeTyped(Coord memory coord) public returns (bytes memory) {
        ...
        MapConfig memory mapConfig = MapConfigComponent(
            getAddressById(components, MapConfigComponentID)
        ).getValue();
    ...
    function canTriggerEncounter(
        ...
        return
            ...
            EncounterableComponent(
                getAddressById(components, EncounterableComponentID)
            ).has(entityId) &&
    ...
    function startEncounter(uint256 entityId) internal returns (uint256) {
        ...
        MonsterType monsterType = MonsterType(
            (rand % uint256(type(MonsterType).max)) + 1
        );
        MonsterTypeComponent(getAddressById(components, MonsterTypeComponentID))
            .set(monsterId, monsterType);
